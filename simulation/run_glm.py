...
client = OpenAI(api_key=ZHIPUAI_API_KEY, base_url=BASE_URL)
...
def save_batch(records: list, batch_num: int, prefix: str = "") -> None:
    stem = f"{prefix}_" if prefix else "glm_batch_"
    path = OUTPUT_DIR / f"{stem}{batch_num:03d}.csv"
...
def main():
    ...
    parser.add_argument("--case", type=str, default=None,
                        choices=["chemistry_long","chemistry_short","geo_long","geo_short",
    ...
    if args.case:
        ...
        case_ids = sorted(c for c in CASES if CASES[c] is not None)
    ...
    casename = args.case or "all"
    prefix = args.case.replace("_", "") if args.case else ""

    print(f"=== GLM-4.5-Air Simulation ({casename}) ===")
    ...
    for case_id in case_ids:
        for proto_letter in PROTOCOLS:
            ...
            for run_num in range(1, RUNS_PER_CASE + 1):
                ...
                if len(buffer) >= 100:
                    save_batch(buffer, batch_num, prefix)
