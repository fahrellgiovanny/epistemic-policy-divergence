# Smoke Test — GPT-5.4 Mini

Runs `math_short / protocol A / 1 run` (15 turns) against GPT-5.4 Mini.
...
Copy to `smoketest_gpt.py`, set `API_KEY`, run:

```bash
python3 smoketest_gpt.py
```

Output: `output/smoketest_gpt.csv`

---

```python
"""Smoke test — GPT-5.4 Mini — math_short / protocol A / 1 run."""
...
out_path = OUTPUT_DIR / "smoketest_gpt.csv"
