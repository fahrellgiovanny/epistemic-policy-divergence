# Gemini-3.1 Flash-Lite Simulation Runner

Runs the session-level hallucination simulation against Google's Gemini.
...
Copy the code block below into `run_gemini.py`, set your API key, and run:

```bash
python3 run_gemini.py
```

Output goes to `output/gemini_batch_*.csv`.
...
def call_api(prompt: str) -> dict:
    """Call Gemini API. Returns dict with rawOutput, finishReason, usageMetadata."""
    ...
    try:
        ...
        usage = {
            "promptTokens": um.prompt_token_count,
            "completionTokens": um.candidates_token_count,
            "totalTokens": um.total_token_count,
        }
    except Exception:
        pass

    return {
        "rawOutput": response.text or "",
